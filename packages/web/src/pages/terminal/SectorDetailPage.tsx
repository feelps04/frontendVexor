...
export default function SectorDetailPage() {
  ...
  const [currentTick, setCurrentTick] = useState<{ symbol: string; bid?: number; ask?: number; last?: number; volume?: number; time?: number } | null>(null);
  const [lastCandlePrices, setLastCandlePrices] = useState<Map<string, number>>(new Map());
  const [traderName, setTraderName] = useState<string>('TRADER')
  const [tradingMode, setTradingMode] = useState<'demo' | 'real'>('demo')
  const [brokerModalOpen, setBrokerModalOpen] = useState(false)
  const [brokerCreds, setBrokerCreds] = useState({ server: '', login: '', password: '' })
    ...
    if (msg.type === 'tick' || msg.type === 'ticks' || msg.type === 'prices') {
      ...
      for (const item of items) {
        ...
        if (item.symbol && displayPrice && Number.isFinite(displayPrice)) {
          ...
          // Atualizar tick atual para o gráfico
          if (selectedSymbol && item.symbol.toUpperCase().startsWith(selectedSymbol.toUpperCase())) {
            setCurrentTick({
              symbol: item.symbol,
              bid: item.bid,
              ask: item.ask,
              last: item.last || displayPrice,
              volume: item.volume,
  ...
  useEffect(() => {
    async function loadSymbols() {
      ...
      setError(null);
      try {
        // Usar Python API via API_BASE para obter símbolos do setor
        const res = await fetch(pythonApiUrl(`/sectors/${encodeURIComponent(sectorId)}/symbols`));
  ...
  // Buscar últimos preços dos candles quando não há tempo real
  useEffect(() => {
    if (symbols.length === 0 || wsConnected) return;
    
    const fetchLastCandlePrices = async () => {
      const newPrices = new Map<string, number>();
      
      // Buscar último preço de cada símbolo (apenas 1 candle)
      for (const s of symbols.slice(0, 20)) { // Limitar a 20 para performance
        try {
          const market = s.symbol.endsWith('.US') ? 'usa' : 'brazil';
          const broker = market === 'usa' ? 'pepperstone' : 'mt5';
          const res = await fetch(pythonApiUrl('/ohlcv'), {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ symbol: s.symbol, timeframe: 'M5', count: 1, broker })
          });
          if (res.ok) {
            const data = await res.json();
            if (data?.data?.[0]?.close) {
              newPrices.set(s.symbol, data.data[0].close);
            }
          }
        } catch (e) {
      ...
      if (newPrices.size > 0) {
        setLastCandlePrices(newPrices);
      }
    };
    
    // Buscar imediatamente e depois a cada 30 segundos
    void fetchLastCandlePrices();
    const interval = setInterval(fetchLastCandlePrices, 30000);
  ...
  const candlePrice = selectedSymbol ? lastCandlePrices.get(selectedSymbol) : null;
  const hasRealPrice = currentPrice != null && Number.isFinite(currentPrice) && currentPrice > 0;
  const displayPrice: number | null = hasRealPrice ? currentPrice! : (candlePrice ?? null);
  ...
  const executeTrade = async () => {
    ...
    if (tradingMode === 'real') {
      try {
        ...
        const apiBase = (window as any).__VEXOR_API_URL__ ?? 'https://api.vexorflow.com';
        const resp = await fetch(`${apiBase}/api/v1/orders`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            userId,
        ...
        if (!resp.ok) {
          ...
          console.error('[REAL ORDER] failed', err);
        }
      } catch (e) {
        console.error('[REAL ORDER] error', e);
      }
      setTradeModal(null);
      setTradeQuantity(1);
      return;
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data }) => {
      const name = data?.user?.user_metadata?.full_name || data?.user?.email?.split('@')[0] || 'TRADER'
      setTraderName(name.toUpperCase())
    })
  }, [])
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr 300px', gap: '16px', minHeight: 'calc(100vh - 120px)' }}>
            ...
            <div style={{ flex: 1, overflow: 'auto', maxHeight: '70vh' }}>
              {loading ? (
                ...
                symbols.map((s) => {
                  ...
                  const candlePrice = lastCandlePrices.get(s.symbol);
                  const displayPrice = quote?.priceBRL || candlePrice;
                    ...
                    >
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <div style={{ textAlign: 'right' }}>
                          <div style={{ fontSize: '14px', fontWeight: 'bold', color: displayPrice ? '#fff' : '#64748b' }}>
                            {displayPrice ? `$ ${displayPrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '—'}
                          </div>
                          {pos && (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                {!selectedSymbol && (
                  <div style={{ height: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#64748b' }}>
                    <div style={{ textAlign: 'center' }}>
                      ...
                      <div>Selecione um ativo para ver o gráfico</div>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div style={{
              ...
              {/* Mode switcher + trader name */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                <div style={{ fontSize: '11px', color: '#00ffc8', fontFamily: 'monospace', fontWeight: 'bold' }}>
                  👤 {traderName}
                </div>
                <div style={{ display: 'flex', gap: '4px' }}>
                  <button
                    ...
                    style={{
                      ...
                      background: tradingMode === 'demo' ? 'rgba(0,255,200,0.15)' : 'rgba(255,255,255,0.05)',
                      border: `1px solid ${tradingMode === 'demo' ? 'rgba(0,255,200,0.5)' : 'rgba(255,255,255,0.1)'}`,
                      borderRadius: '4px', color: tradingMode === 'demo' ? '#00ffc8' : '#64748b', cursor: 'pointer',
                    }}
                  >DEMO</button>
                  <button
                    ...
                    style={{
                      padding: '4px 10px', fontSize: '10px', fontWeight: 'bold',
                      background: tradingMode === 'real' ? 'rgba(251,191,36,0.15)' : 'rgba(255,255,255,0.05)',
                      border: `1px solid ${tradingMode === 'real' ? 'rgba(251,191,36,0.5)' : 'rgba(255,255,255,0.1)'}`,
                      borderRadius: '4px', color: tradingMode === 'real' ? '#fbbf24' : '#64748b', cursor: 'pointer',
                    }}
                  >{tradingMode === 'real' ? '🟡 REAL' : 'REAL'}</button>
                </div>
              </div>
        ...
        {brokerModalOpen && (
          ...
          >
            <motion.div
              ...
              style={{ background: 'rgba(15,15,15,0.98)', border: '1px solid rgba(251,191,36,0.3)', borderRadius: 12, padding: '28px', width: 420, maxWidth: '90vw' }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                <h2 style={{ fontSize: '18px', fontWeight: 'bold', color: '#fbbf24' }}>🔗 Conectar Corretora</h2>
                <button onClick={() => setBrokerModalOpen(false)} style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer' }}><X size={20} /></button>
              </div>
              <div style={{ fontSize: '13px', color: '#94a3b8', marginBottom: '20px', padding: '12px', background: 'rgba(251,191,36,0.05)', border: '1px solid rgba(251,191,36,0.1)', borderRadius: 6 }}>
                Conecte sua conta MetaTrader 5 para operar com dinheiro real. Suas credenciais são criptografadas e nunca armazenadas em texto plano.
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '20px' }}>
                {[
                  { label: 'Servidor MT5', key: 'server', placeholder: 'ex: Genial-Server' },
                  { label: 'Login (número da conta)', key: 'login', placeholder: 'ex: 123456789' },
                  ...
                  <div key={key}>
                    ...
                    <input
                      ...
                      value={brokerCreds[key as keyof typeof brokerCreds]}
                      onChange={(e) => setBrokerCreds(prev => ({ ...prev, [key]: e.target.value }))}
                      style={{ width: '100%', padding: '10px 12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 6, color: '#fff', fontSize: '14px', boxSizing: 'border-box' }}
                    />
                  </div>
                ))}
              </div>
              <button
                onClick={() => {
                  if (brokerCreds.server && brokerCreds.login && brokerCreds.password) {
                    setTradingMode('real')
                    setBrokerModalOpen(false)
                    alert('✅ Corretora conectada! As ordens serão enviadas ao seu MetaTrader 5.')
                  }
                }}
                style={{ width: '100%', padding: '14px', background: '#fbbf24', border: 'none', borderRadius: 8, color: '#000', fontSize: '15px', fontWeight: 'bold', cursor: 'pointer' }}
              >
                🔗 CONECTAR E OPERAR REAL
              </button>
              <div style={{ marginTop: '12px', fontSize: '10px', color: '#475569', textAlign: 'center' }}>
                Modo real envia ordens diretamente ao MT5 via EA. Certifique-se de ter o Expert Advisor ativo.
              </div>
            </motion.div>
          </motion.div>
        )}
